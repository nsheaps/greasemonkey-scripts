 ...
 */
function repoConfigTargetKey(target) {
 ...
 */
function fetchRepoConfigForTarget(target) {
    const atRef = fetchRepoConfig(target.org, target.repo, target.branch);
    if (!target.fallBackToDefaultBranch || target.branch === undefined)
        return atRef;
 ...
 */
function branchFromTreeHref(href, org, repo) {
    const match = href.match(/^\/([^/]+)\/([^/]+)\/tree\/(.+)$/);
    if (!match)
        return null;
...
function prHeadBranchHref() {
    for (const link of document.querySelectorAll(PR_BRANCH_NAME_SELECTOR)) {
        for (let sibling = link.nextElementSibling; sibling; sibling = sibling.nextElementSibling) {
            if (sibling.getAttribute("data-component") === "IconButton")
 ...
 */
function scrapePrHeadBranch(context) {
    if (context.kind !== "pr")
        return undefined;
...
function repoConfigTarget(context, pathname) {
    ...
    if (prHeadBranch)
        return { ...target, branch: prHeadBranch, fallBackToDefaultBranch: true };
